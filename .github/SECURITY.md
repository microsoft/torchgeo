...
- **Insecure usage of our dependencies.** For example, the use of `eval`[^1], `exec`, `pickle.loads`, `subprocess.run(shell=True)`, `numpy.load(allow_pickle=True)`, or `torch.load(weights_only=False)`[^2] are standard features of Python, NumPy, and PyTorch. But without input validation, these can result in arbitrary code execution.
