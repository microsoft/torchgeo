...
class BConvLSTM(nn.Module):
    ...
    Runs a :class:`ConvLSTM` in both the forward and reverse temporal
    directions and concatenates the final cell states.
    ...
    * https://arxiv.org/abs/2107.07933
    ...
    def __init__(
        ...
        """Initialize BConvLSTM.

        Args:
            input_dim: Number of channels in the input.
            hidden_dim: Number of channels in the hidden state.
            kernel_size: Size of the convolutional kernel.
            input_size: Unused; accepted for API compatibility.
        """
        super().__init__()
    ...
    def forward(
        ...
        Args:
            ...
            pad_mask: Boolean mask of shape ``(B, T)`` where ``True`` marks
                padded (invalid) timesteps.

        Returns:
            Output tensor of shape ``(B, 2 * hidden_dim, H, W)``.
        """
        x_fwd = x
        if pad_mask is not None:
            x_fwd = x.masked_fill(pad_mask[:, :, None, None, None], 0.0)
        _, forward_states = self.convlstm_forward.forward_features(x_fwd)
        out = forward_states[0][1]  # last cell state: (B, hidden_dim, H, W)

        x_reverse = torch.flip(x, dims=[1])
        if pad_mask is not None:
            pmr = torch.flip(pad_mask.float(), dims=[1]).bool()
            x_reverse = x_reverse.masked_fill(pmr[:, :, None, None, None], 0.0)
        _, backward_states = self.convlstm_backward.forward_features(x_reverse)

        return torch.cat([out, backward_states[0][1]], dim=1)
