    ...
    * https://arxiv.org/pdf/2011.09766.pdf
    ...
    def __init__(
        ...
        self.backbone = getattr(resnet,
                                backbone)(pretrained=backbone_pretrained)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        features = self.fpn(
            OrderedDict({f"c{i + 2}": features[i]
                         for i in range(4)}))
...
class FSRelation(nn.Module):
    ...
    def forward(self, scene_feature: torch.Tensor,
                features: List[torch.Tensor]) -> List[torch.Tensor]:
        """Forward pass of the model."""
        # [N, C, H, W]
        content_feats = [
            c_en(p_feat)
