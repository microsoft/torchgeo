...
class RecUNet(nn.Module):
    """Recurrent U-Net for satellite image time series.

    Replaces the L-TAE in :class:`UTAE` with a recurrent encoder
    (mean, LSTM, or bidirectional LSTM) and uses temporal averaging for
    skip connections.
    ...
    * https://arxiv.org/abs/2107.07933
    ...
    def __init__(
        ...
        Args:
            input_dim: Number of channels in the input images.
            encoder_widths: Number of channels at each encoder stage.
            decoder_widths: Number of channels at each decoder stage.
            out_conv: Channel widths for the final output convolutions.
            ...
            temporal: Temporal encoder type. One of ``'mean'``, ``'lstm'``,
                ``'blstm'``, or ``'mono'``.
            input_size: Spatial size of the input (used to compute LSTM dims).
            encoder_norm: Normalisation type for the encoder.
            hidden_dim: Hidden dimension for LSTM-based encoders.
            encoder: If ``True``, return feature maps instead of class scores.
            padding_mode: Spatial padding strategy for convolutions.
        ...
        if temporal == 'mean':
            ...
            self.temporal_encoder = ConvLSTM(
                input_dim=self.encoder_widths[-1],
            ...
            self.out_convlstm = nn.Conv2d(
                ...
                out_channels=self.encoder_widths[-1],
                kernel_size=3,
                padding=1,
            )
        elif temporal == 'blstm':
    ...
    def forward(
        self, x: Tensor, batch_positions: Tensor | None = None
    ) -> Tensor | tuple[Tensor, list[Tensor]]:
        """Forward pass.

        Args:
            x: Input tensor of shape ``(B, T, C, H, W)``.
            batch_positions: Unused; accepted for API compatibility.

        Returns:
            Output tensor, and optionally intermediate feature maps.
        """
        pad_mask = (x == self.pad_value).all(dim=-1).all(dim=-1).all(dim=-1)  # B x T
        ...
        if self.temporal == 'mean':
            ...
            x_lstm = feature_maps[-1]
            if pad_mask.any():
                x_lstm = x_lstm.masked_fill(pad_mask[:, :, None, None, None], 0.0)
            _, last_states = self.temporal_encoder.forward_features(x_lstm)  # type: ignore[union-attr]
            out = last_states[0][1]  # last cell state
