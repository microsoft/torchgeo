...
class TSViT(nn.Module):
    ...
    * https://arxiv.org/abs/2301.11942
    ...
    def __init__(
        ...
        # -1 is set to exclude the time feature channel during embedding
        ...
        self.temporal_transformer = Transformer(
            self.dim, self.temporal_depth, self.heads, self.dim_head, self.dim * self.scale_dim, dropout
        )
        
        self.space_pos_embedding = nn.Parameter(torch.randn(1, num_patches, self.dim))
        self.space_transformer = Transformer(
            self.dim, self.spatial_depth, self.heads, self.dim_head, self.dim * self.scale_dim, dropout
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        # Original format expected by author logic: [B, T, H, W, C]
        x = x.permute(0, 1, 4, 2, 3)  
        B, T, C, H, W = x.shape

        # Extract time channel and create position encodings
        xt = x[:, :, -1, 0, 0]
        x = x[:, :, :-1]
        ...
        # Spatiotemporal sequence encoding
        ...
        x = rearrange(
            x, 'b c (h1 w1) (p1 p2) -> b c (h1 p1) (w1 p2)', 
