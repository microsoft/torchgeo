...
class OSCDDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        train_transf = Compose([self.preprocess, random_crop])
        transf = Compose([self.preprocess])

        dataset = OSCD(
            self.root_dir, split="train", bands=self.bands, transforms=train_transf
        )
        ...
        if self.val_split_pct > 0.0:
            ...
            self.val_dataset.dataset.transforms = transf  # type: ignore[attr-defined]
        else:
            self.train_dataset = dataset  # type: ignore[assignment]
            self.val_dataset = None  # type: ignore[assignment]

        self.test_dataset = OSCD(
            self.root_dir, split="test", bands=self.bands, transforms=transf
        )
