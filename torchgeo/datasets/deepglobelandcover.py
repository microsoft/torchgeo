...
class DeepGlobeLandCover(VisionDataset):
    ...
    * masks are RGB image with with unique RGB values representing the class
    ...
    where id is an integer assigned to every image. 
    ...
    * DeepGlobe 2018: A Challenge to Parse the Earth Through Satellite Images
    ...
    def __init__(
        ...
        self.files = []
        for image in os.listdir(os.path.join(root, self.data_root, split_folder, "images")):
            if image.endswith(".jpg"):
                id = image[:-8]
                image_path = os.path.join(root, self.data_root, split_folder, "images", image)
                mask_path = os.path.join(root, self.data_root, split_folder, "masks", str(id)+"_mask.png")
