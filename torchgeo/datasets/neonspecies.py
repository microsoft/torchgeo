...
class NEONTreeSpecies(VisionDataset):
    ...
    The Weecology Lab at the University of Florida has developed a species prediction benchmark using data from the National Ecological Observatory Network `<https://idtrees.org/>`_
    dataset is a dataset for tree crown detection.

    Dataset features:

    * RGB Image, Canopy Height Model (CHM), Hyperspectral Image (HSI), LiDAR-derived CHM model
    ...
    * Task 1 - object classification (species classification)
    * Train set contains X images
    * Test set (task 1) contains X images
    ...
    * shapefiles (.shp) containing field collected data on tree stems from NEON's Woody Vegetation Structure Dataset woody <https://data.neonscience.org/data-products/DP1.10098.001>
