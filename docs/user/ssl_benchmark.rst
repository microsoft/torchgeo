...
.. list-table::
   ...
   * - :class:`~torchgeo.tasks.MoCo` v3
     ...
     - **0.9494**
     - 1e-2
     - `moco_resnet50_lr0p01 <https://github.com/torchgeo/torchgeo/blob/main/experiments/eurosat_benchmarking/configs/resnet50/moco_resnet50_lr0p01.yaml>`__
   * - :class:`~torchgeo.tasks.MoCo` v3
     - ViT-S/16
     - 0.9413
     - 1e-4
     - `moco_vit_small_lr0p0001 <https://github.com/torchgeo/torchgeo/blob/main/experiments/eurosat_benchmarking/configs/vit_small/moco_vit_small_lr0p0001.yaml>`__
   * - :class:`~torchgeo.tasks.SimCLR`
     - ResNet-50
     - 0.9356
     - 1.5
     - `simclr_resnet50_lr1p5 <https://github.com/torchgeo/torchgeo/blob/main/experiments/eurosat_benchmarking/configs/resnet50/simclr_resnet50_lr1p5.yaml>`__
   * - :class:`~torchgeo.tasks.SimCLR`
     - ViT-S/16
     - 0.9296
     - 1.5
     - `simclr_vit_small_lr1p5 <https://github.com/torchgeo/torchgeo/blob/main/experiments/eurosat_benchmarking/configs/vit_small/simclr_vit_small_lr1p5.yaml>`__
   * - :class:`~torchgeo.tasks.BYOL`
     - ResNet-50
     - 0.9294
     - 1e-3
     - `byol_resnet50_lr0p001 <https://github.com/torchgeo/torchgeo/blob/main/experiments/eurosat_benchmarking/configs/resnet50/byol_resnet50_lr0p001.yaml>`__
   * - :class:`~torchgeo.tasks.BYOL`
     - ViT-S/16
     - 0.9241
     - 1e-4
     - `byol_vit_small_lr0p0001 <https://github.com/torchgeo/torchgeo/blob/main/experiments/eurosat_benchmarking/configs/vit_small/byol_vit_small_lr0p0001.yaml>`__

The configuration for every run in the sweep, including the learning rates that are not selected above, lives in `experiments/eurosat_benchmarking <https://github.com/torchgeo/torchgeo/tree/main/experiments/eurosat_benchmarking>`__ alongside the scripts that produced and scored it.
...
The other runs in the sweep differ only in ``model.init_args``: the task's ``class_path``, its ``model``, and its ``lr``. Rather than reproducing all of them here, the full grid of 24 configurations — three tasks, two encoders, four learning rates each — is committed under `experiments/eurosat_benchmarking/configs <https://github.com/torchgeo/torchgeo/tree/main/experiments/eurosat_benchmarking/configs>`__ and is generated by ``make_configs.py`` in the same directory, so the grid can be extended without hand-editing YAML. Every configuration uses ``in_channels: 13`` and the same ``trainer`` and ``data`` blocks as above.
