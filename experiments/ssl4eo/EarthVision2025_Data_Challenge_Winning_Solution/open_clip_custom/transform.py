...
def image_transform_n_dim(
    #
    #Transforms for raw tensor (no jitter, etc)
    #
        image_size: Union[int, Tuple[int, int]],
        is_train: bool,
        num_channels: int,
    ...
    # Ensure mean and std are provided for all channels
    # TODO: enter mean and std from the ssl4eos repo
    mean = mean or (1.,) * num_channels # normalizing in dataclass
    std = std or (1.,) * num_channels

    if len(mean) != num_channels or len(std) != num_channels:
        raise ValueError(f"Mean and std must have {num_channels} values for normalization.")
    ...
    if is_train:
        ...
        # if aug_cfg.color_jitter_prob:
        #     assert aug_cfg.color_jitter is not None and len(aug_cfg.color_jitter) == 4
        #     train_transform.append(color_jitter(*aug_cfg.color_jitter, p=aug_cfg.color_jitter_prob))
        # if aug_cfg.gray_scale_prob:
        #     train_transform.append(gray_scale(aug_cfg.gray_scale_prob))
        train_transform.extend([
            normalize,
        ])
...
def image_transform_tensor(
    ...
    return image_transform_n_dim(
        ...
        # fill_color=cfg.fill_color,
        aug_cfg=aug_cfg,
        num_channels=108 # TODO: fix hardcoded value
    )