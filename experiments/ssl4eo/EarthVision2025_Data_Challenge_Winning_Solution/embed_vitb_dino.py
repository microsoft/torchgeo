...
parser.add_argument("--output-path", "-o", type=str, default="embedding_vit_base_finetune_epoch0030.csv", help="Path to the output CSV file")
...
patch_size = 16
model = vits.__dict__[f'{architecture}'](patch_size=patch_size, num_classes=0)
print(f"Model {architecture} {patch_size}x{patch_size} built.")


model.cuda()
model.eval()
if os.path.isfile(f'{pretrained_weights}'):
    state_dict = torch.load(f'{pretrained_weights}', map_location="cpu", weights_only=False)
    if "teacher" and "teacher" in state_dict:
    ...
    print('Pretrained weights found at {} and loaded with msg: {}'.format(f'{pretrained_weights}', msg))
elif args.arch == "vit_small" and patch_size == 16:
...
with torch.no_grad():
    ...
    for ind, data_file_name in enumerate(train_loader):
        ...
        features = model(images)  # Get features (not the projection head output)
        features = features.cpu().numpy()
        # print(features.shape)
        # Resize feature shape to (10, 1024) by concatenating itself multiple times
