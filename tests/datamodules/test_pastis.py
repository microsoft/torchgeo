...
class TestPASTIS100DataModule:
    @pytest.fixture
    def datamodule(self, tmp_path: Path) -> PASTIS100DataModule:
        src = os.path.join('tests', 'data', 'pastis', 'PASTIS-R')
        dst = tmp_path / 'PASTIS-R-100'
        shutil.copytree(src, dst)
        return PASTIS100DataModule(
            root=tmp_path, batch_size=2, num_workers=0, mode='semantic'
        )

    def test_train_dataloader(self, datamodule: PASTIS100DataModule) -> None:
        datamodule.setup('fit')
        next(iter(datamodule.train_dataloader()))

    def test_val_dataloader(self, datamodule: PASTIS100DataModule) -> None:
        datamodule.setup('validate')
        next(iter(datamodule.val_dataloader()))

    def test_test_dataloader(self, datamodule: PASTIS100DataModule) -> None:
        datamodule.setup('test')
        next(iter(datamodule.test_dataloader()))
