...
class TestCopernicusPretrain:
    ...
    def dataset(self) -> wds.WebDataset:
        ...
        numbers = sorted(set(re.findall(r'example-(\d{6})\.tar', f)[0] for f in tar_files))
        start, end = numbers[0], numbers[-1]
        shards = f"example-{{{start}..{end}}}.tar"
        shards_path = os.path.join(root, shards)
        copernicuspretrain = CopernicusPretrain(shards_path, shuffle=0, shardshuffle=False, resampled=False)
