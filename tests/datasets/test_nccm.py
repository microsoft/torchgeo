...
class TestNCCM:
    ...
    def dataset(self, monkeypatch: MonkeyPatch, tmp_path: Path) -> NCCM:
        ...
        url = os.path.join(
            "tests", "data", "nccm", "CDL{}_clip.zip" #imp doubt here: now in the case of this dataset, we have only 1 zip folder for all years combined. there is no zip for each year. what do we do in this case?
        )
    ...
    def test_already_downloaded(self, tmp_path: Path) -> None:
        pathname = os.path.join(
            "tests", "data", "nccm", "CDL2017_clip.zip" #cant use this because we have only one zip!
        )
        root = str(tmp_path)
