...
class Args:
    ...
    def __init__(self) -> None:
        ...
        self.normalizing_factor: np.ndarray[Any, np.dtype[np.float]] = np.array([[0, 0.5, 1.0]])
        self.mask_2d: bool = True
        self.model_type: str = "swin"
        self.timepoint_aggregation: str = "concat"
        self.means: np.ndarray[Any, np.dtype[np.float]] = np.array([0.485, 0.456, 0.406])
        self.stds: np.ndarray[Any, np.dtype[np.float]] = np.array([0.229, 0.224, 0.225])
