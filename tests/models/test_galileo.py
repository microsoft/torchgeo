...
SPACE_TIME_GROUPS = OrderedDict({'g0': list(range(SPACE_TIME_BANDS))})

SPACE_GROUPS = OrderedDict({'g0': list(range(SPACE_BANDS))})

TIME_GROUPS = OrderedDict({'g0': list(range(TIME_BANDS))})

STATIC_GROUPS = OrderedDict({'g0': list(range(STATIC_BANDS))})


def make_inputs(
    batch: int = 2, H: int = 64, W: int = 64, T: int = 2
...
@pytest.fixture
def mocked_weights(
    tmp_path: Path, monkeypatch: MonkeyPatch, load_state_dict_from_url: Any
) -> GalileoWeights:
